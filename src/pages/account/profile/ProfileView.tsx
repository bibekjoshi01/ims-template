...
export default function ProfileView({ setEdit }: { setEdit: () => void }) {
  ...
  const fieldOrder: (keyof UserProfile)[] = ['fullName', 'username', 'email', 'phoneNo', 'lastLogin', 'dateJoined', 'isEmailVerified', 'roles'];
    ...
    <Grid container spacing={2} sx={{ my: 1 }}>
      <Grid item xs={12} md={6}>
        <MainCard divider title="Personal Information">
          <Avatar src={typeof profile?.photo === 'string' ? profile.photo : undefined} alt="Profile" sx={{ width: 100, height: 100, mb: 2 }} />
